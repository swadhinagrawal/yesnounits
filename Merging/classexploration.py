...
class Decision_making:
    def __init__(self,number_of_options,err_type,mu_assessment_err,sigma_assessment_err):
        ...
        self.yes_stats = np.zeros((self.number_of_options,self.number_of_options))
        self.y_ratios = []
    ...
    def hypothesis_testing(self,pc):
        for i in range(self.number_of_options-1):
            for j in range(i+1,self.number_of_options):
                val = pt([self.votes[i],self.votes[j]],[pc[i],pc[j]],verbose=False)
                flag = 0
                for k in val:
                    if val[0][0]== np.nan or val[1]<0:
                        flag = 1
                        break
                if val[0][0]!= np.nan and val[1]>0 and flag!=1:
                    self.yes_stats[i,j] = val[0][1]
                    self.yes_stats[j,i] = val[0][1]
                else:
                    self.yes_stats[i,j] = 1
                    self.yes_stats[j,i] = 1

    def best_option_sim(self,ref_highest_quality):
        """
        ...
        if available_opt[opt_choosen] ==  ref_highest_quality:
            return [1,opt_choosen]
        else:
            return [0,opt_choosen]
...
class Qranking:
    ...
    def ranking(self,oq,y_ratios):
        for i in range(len(oq)):
            for j in range(len(oq)):
                if oq[i]>oq[j] or i == j:
                    self.ref_rank[i,j] = 0
                elif oq[i]<oq[j]:
                    self.ref_rank[i,j] = 1
                ...
                if y_ratios[i]>y_ratios[j] or i == j:
                    self.exp_rank[i,j] = 0
    ...
    def wrong_ranking_cost(self):
        self.TW = np.subtract(self.exp_rank,self.ref_rank)
        self.SW = np.subtract(self.exp_rank,self.ref_rank)
        self.TW[self.TW<=1] = 0
        self.TW[self.TW>1] = 1
        self.SW[self.SW>1] = 0
        self.SW[self.SW<0] = 1
        ...
        for i in range(self.n):
            for j in range(self.n):
                measure_of_incorrectness += self.SW[i,j] + self.TW[i,j]
...
class workFlow:
    ...
    def majority_decision(self,number_of_options,Dx,assigned_units,ref_highest_quality,pc,mu_assessment_err=0,sigma_assessment_err=0,\
        ...
        majority_dec = DM.best_option_sim(ref_highest_quality)
        qr = Qranking(number_of_options)
        qr.ranking(Dx,DM.y_ratios)
        wrc = qr.wrong_ranking_cost()

        ranks = np.round(np.sum(qr.ref_rank,axis=1),decimals=0)
        
        p_vals = DM.yes_stats[ref_highest_quality]
        
        ties = []
        for i in range(len(p_vals)):
            ties.append([ranks[i],p_vals[i]])
        ties = sorted(ties,key = lambda x:x[0])
        if quorum == None:
            # plt.scatter(Dx,DM.votes)
            # plt.show()
            return majority_dec[0],wrc,ties[1][1],ties
...
class Prediction:
    ...
    @staticmethod
    def optimization(x,y,z,max_iter=10000,d = 6):
        iterations = 0
        ...
        while iterations<max_iter:
            ...
            if thisgoodness > goodness_of_fit and points<=len(maybeInliers_z):
                bestFit = params
                points = len(maybeInliers_z)
...
class Visualization:
    def data_visualize(self,file_name,save_plot,x_var_,y_var_,plot_type,z_var_,gaussian=1,uniform=0,cbar_orien=None,line_labels=None,sigma_x_1=None,\
        ...
        if plot_type == 'graphics':
            if 'mu' in x_var_:
                ...
                HRCC=[None,None,None]
            if 'mu' not in x_var_:
                self.graphicPlot(a= y,b=x,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD.png',cbar_loc=cbar_orien,z_var=z,z_max_fit = HRCC[0],z_max_fit_lab=HRCC[1],line_labels=line_labels)
            if gaussian ==1:
                ...
                # predicted_hrcc = prd.Hrcc_predict(delta_mu,x_var_,x,y,z,sigma_x_1,sigma_x_2,line_labels,prd.gaussian,prd.ICPDF,1.0-((3.8/(2*line_labels))),prd.z_extractor,prd.optimization,line_labels)
                # d = np.round(abs(predicted_hrcc[0][1]-HRCC[0][1])/ np.sqrt(HRCC[0][0]**2 +1),decimals=2)
                # delta_slope = np.round(abs(predicted_hrcc[0][0]-HRCC[0][0]),decimals=2)
                # intercepts = [HRCC[0][1],predicted_hrcc[0][1]]
            ...
            if uniform ==1:
                ...
                # # ES2M non integral
                # predicted_hrcc = prd.Hrcc_predict(delta_mu,x_var_,x,y,z,sigma_x_1,sigma_x_2,line_labels,prd.uniform,prd.ICPDF,1.0-(9/(2*line_labels)),prd.z_extractor,prd.optimization,line_labels)
                # d = np.round(abs(predicted_hrcc[0][1]-HRCC[0][1])/ np.sqrt(HRCC[0][0]**2 +1),decimals=2)
                # delta_slope = np.round(abs(predicted_hrcc[0][0]-HRCC[0][0]),decimals=2)
                # intercepts = [HRCC[0][1],predicted_hrcc[0][1]]
                # self.graphicPlot(a= y,b=x,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD_ES2M.pdf',cbar_loc=cbar_orien,z_var=z,z_max_fit = HRCC[0],z_max_fit_lab=HRCC[1],options_line=[predicted_hrcc[0]],line_labels=[line_labels,predicted_hrcc[1]],d=d,delta_slope=delta_slope,intercepts = intercepts)
    ...
    def graphicPlot(self,a,b,x_name,y_name,z_name,title,save_name,cbar_loc,z_var,z_max_fit=None,z_max_fit_lab = None,options_line=None,line_labels=None,d=None,delta_slope=None,intercepts = None):
        points = []
        fig, ax = plt.subplots()
        ...
        if isinstance(options_line, type(None)) == False:
            for j in range(len(options_line)):
                ...
                plt.plot(b,ESM,color = colors[j],linestyle='-',label = 'HRCC = '+str(line_labels[1])+', d = '+str(d)+', $\Delta m$ = '+str(delta_slope)+', b = '+str(np.round(intercepts[1],decimals=1)),linewidth=0.8)
        if isinstance(z_max_fit, type(None)) == False:
            z_best_fit = [z_max_fit[0]*bb+z_max_fit[1] for bb in b]
            plt.plot(b,z_best_fit,color = 'red',label = 'HRCC = '+str(z_max_fit_lab)+', b = '+str(np.round(intercepts[0],decimals=1)),linewidth=0.8)
