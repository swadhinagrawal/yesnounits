...
if __name__=="__main__":
	path = os.getcwd() + "/results_democracy/"
	run_no = 2
	...
	data_files = np.sort(np.array([f for f in os.listdir(path) if '.csv' in f and f[:len(file_identifier[run_no])] == file_identifier[run_no] and len(f)>10]))
	...
	plt.savefig(path+"0_no_agents_plot.png",format = "png",bbox_inches="tight",pad_inches=0.2)
	plt.show()
