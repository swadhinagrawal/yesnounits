...
def looper(p,vP,path,no_units):
    for i in range(200):
        ...
        p.num_robots = np.sum(robots_to_assign)
        ...
        if isinstance(model.best_option,type(None)) == False and isinstance(decision.ref_best,type(None)) == False:
            out = pd.DataFrame(data=[{'$x_{max}$ opt No.':decision.ref_best,'$x_{max}$':options[decision.ref_best].quality,'$CDM$ opt No.':model.best_option,'$CDM$':options[model.best_option].quality,'Iterations':model.iterations,'No_units':no_units}],columns=vP.data_columns_name)
            print({'$x_{max}$ opt No.':decision.ref_best,'$x_{max}$':options[decision.ref_best].quality,'$CDM$ opt No.':model.best_option,'$CDM$':options[model.best_option].quality,'Iterations':model.iterations,'No_units':no_units})
        else:
            out = pd.DataFrame(data=[{'$x_{max}$ opt No.':model.ref_best,'$x_{max}$':None,'$CDM$ opt No.':model.best_option,'$CDM$':None,'Iterations':model.iterations,'No_units':no_units}],columns=vP.data_columns_name)
            print({'$x_{max}$ opt No.':decision.ref_best,'$x_{max}$':None,'$CDM$ opt No.':model.best_option,'$CDM$':None,'Iterations':model.iterations,'No_units':no_units})
