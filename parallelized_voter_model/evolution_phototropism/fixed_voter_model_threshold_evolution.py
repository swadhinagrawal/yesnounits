...
class variableParams:
	def __init__(self):
		...
		self.data_columns_name = ['n','$\mu_{m_1}$','$\sigma_{m_1}$','$\mu_{h_{pred}}$','$\sigma_{h_{pred}}$','$\mu_{h_1}$','$\sigma_{h_1}$','$x_{max}$ opt No.','$x_{max}$','$CDM$ opt No.','$CDM$','Iterations','no:yes','after_replication_no:yes']
...
class Params:
	...
	def packaging(self):
		...
		self.mu_h_pred = vP.ICPDF(1-(1/self.num_opts),self.mu_x,self.stop_x,self.step,self.x,self.pdf)
		choices_h = [['G',(0.12*np.log10(self.num_opts) + 0.56)*self.sigma_x_1],['U',self.sigma_x_1*np.exp(-0.07*self.num_opts+0.7)],['K',(0.12*np.log10(self.num_opts) + 0.54)*self.sigma_x_1]]
		[_,self.sigma_h_pred] = self.functionChooser([self.Dh,self.sigma_h_1],choices_h)
					...
					while (r.no_count/(r.yes_count+0.01)) - (p.num_opts - 1)<=0:
						...
						if (r.no_count/(r.yes_count+0.01)) - (p.num_opts - 1)<=0:
							...
							if robots[self.yes_units[child].id].response==1 and child != r.id:
								...
								robots[self.yes_units[child].id].threshold += 0.7#np.random.uniform(low = 0.5,high=1)#p.h_distribution_fn(m_units=1,mu_h=List([r.threshold,r.threshold]),sigma_h=p.sigma_h)[0]
					...
					while (r.yes_count/(r.no_count+0.01)) - (p.num_opts - 1)<=0:
						...
						if (r.yes_count/(r.no_count+0.01)) - (p.num_opts - 1)<=0:
							...
							if robots[child].response==0 and child != r.id:
								...
								# We can also multiply this subtraction value with change in number of options (Idea)
								robots[child].threshold -= 0.7#np.random.uniform(low = 0.5,high=1)#p.h_distribution_fn(m_units=1,mu_h=List([r.threshold,r.threshold]),sigma_h=p.sigma_h)[0]
