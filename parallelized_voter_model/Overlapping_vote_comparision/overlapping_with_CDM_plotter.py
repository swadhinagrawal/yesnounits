...
if __name__=="__main__":
	...
	CDM_data_file = np.sort(np.array([f for f in os.listdir(path) if '.csv' in f and f[:3] == '480' and len(f)>10]))[0]
	...
	mu_h_pred = vp.ICPDF(1-(1/10),List([mux[0],mux[0]]),stop,step,x,pdf)
	...
	for i in range(1,10,1):
		ESM = vp.ICPDF(float(i)/10,mean_x,stop,step,x,pdf)
		slices.append(np.round(ESM,decimals=3))
