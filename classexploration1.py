...
class Visualization:
    def data_visualize(self,file_name,save_plot,x_var_,y_var_,z_var_,plot_type,gaussian=1,uniform=0,cbar_orien=None,line_labels=None,sigma_x_1=None,\
        ...
        for i in opt_var:
            ...
            if plot_type!='image':
                # x,y,z for only HRCC
                if i[z_var_] >= z_max-0.08*(num_of_opts**(1/5))/(mu_m**(1/5)): # mu_h=mu_pred sigma= 0.015*(num_of_opts**(0.5))/(mu_m**(1/10)),0.02*(num_of_opts**(0.5)),mu = 0.05*(num_of_opts**(1/5))/(mu_m**(1/5)), when sigmax=0.3; 0.08*(num_of_opts**(1/5))/(mu_m**(1/5))
        ...
        if plot_type == 'graphics':
            ...
            if 'mu' not in x_var_:
                ...
                self.graphicPlot(a= y,b=x,x_name=r'%s'%(x_var_[:-4]+x_var_[-2:]),y_name=r'%s'%(y_var_[:-4]+y_var_[-2:]),z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD.eps',cbar_loc=cbar_orien,z_var=z,z_max_fit = HRCC[0],line_labels=line_labels,z_max_fit_lab=HRCC[1],min_sig_h=min_sig_h,intercepts = [HRCC[0][1]])
    ...
    def graphicPlot(self,a,b,x_name,y_name,z_name,title,save_name,cbar_loc,z_var,z_max_fit=None,z_max_fit_lab = None,options_line=None,line_labels=None,d=None,delta_slope=None,intercepts = None,min_sig_h=None):
        ...
        if isinstance(options_line, type(None)) == False:
            for j in range(len(options_line)):
                ...
                plt.plot(b,ESM,color = colors[j],linestyle='-',label = 'Prediction: HARS = '+str(line_labels[1])+', d = '+str(d)+', $\Delta m$ = '+str(delta_slope)+', b = '+str(np.round(intercepts[1],decimals=1)),linewidth=2)
        if isinstance(z_max_fit, type(None)) == False:
            z_best_fit = [z_max_fit[0]*bb+z_max_fit[1] for bb in b]
            plt.plot(b,z_best_fit,color = 'red',label = 'Fitting: HARS = '+str(z_max_fit_lab)+', m = '+str(np.round(z_max_fit[0],decimals=1))+', b = '+str(np.round(intercepts[0],decimals=1)),linewidth=2)
            # if 'mu' in x_name:
    ...
    for j in range(len(var_mu)):
        ...
        for k in range(len(var_sigma)):
            ...
            for nop in range(len(number_of_options)):
                axs[nop,2*k].plot(dis_x,pdf,c='black',linewidth = 1.5)
                ...
                axs[nop,2*k].axvline(mean_esmes2m,0,500,color='red',label = r'$\bf \mu_{ESM,ES2M}$',linewidth = 1.5,linestyle='--')
                axs[nop,2*k].legend(loc='upper right',frameon=False)
                axs[nop,2*k+1].axvline(mean_esmes2m,0,500,color='red',label = r'$\bf \mu_{ESM,ES2M}$',linewidth = 1.5,linestyle='--')
                ...
                axs[nop,2*k].title.set_color('black')
