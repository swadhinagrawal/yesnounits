...
if bimodal_x_normal_h_plotter==1:
    ...
    for i in mum_bxgh:
        ...
        for nop in range(len(number_of_opts_bxgh)):
            ...
            ax[nop].set_xlabel('$\mu_{x}$',fontsize = 18)
            ax[0].set_ylabel('$\mu_{h}$',fontsize = 18)
            ...
            ax[nop].set_title(title,font,y=-0.23,color=(0.3,0.3,0.3,1))
            ...
            vis.graphicPlot_paper(a,b,x_name,y_name,z_name,title,save_name,z_var,fig, ax[nop],cbar,[slope,intercept],hars,[predicted_hrcc[0]],line_labels,d,delta_slope,intercepts,linestyle=line_style[nop])
...
if normal_x_normal_h_plotter==1:
    ...
    for i in mum_bxgh:
        ...
        fig.legend(lines, labels,loc='upper left',prop=dict(weight='bold',size=12), bbox_to_anchor=(0.15, 0.83),labelcolor=(0.3,0.3,0.3,1),ncol=3,frameon=False)
        plt.show()
        mum_slopes_bxgh.append(num_slopes_gxgh)
...
if reflection_plot == 1:
    ...
    for sigma_ in range(len(sigmas)):
        ...
        for nop in range(len(number_of_options)):
            ...
            axs[sigma_].axvline(mean_esmes2m,0,500,color='red',label = r'$\bf \mu_{ESM,ES2M}$',linewidth = 1,linestyle="--")
            ...
            axs[sigma_].plot(dis_x,pdf_x,color='black',linewidth=3,label=r'$G_x$')
            ...
            #     axs[sigma_].scatter(options_quality,(0.3-i*0.1)*np.ones_like(options_quality),s=9,edgecolor = 'black')
            #     if sigma_==0:
            #         axs[0].text(start1-2,(0.32-i*0.1),'trial '+str(i),font,color='black')
            #         axs[0].text(start1-2,0.05,r'$\bf \mu_x$',font,color='black') 

            axs1 = axs[sigma_].twinx()
            axs1.plot(dis_h,pdf_h,color='maroon',linewidth=3,label=r'$G_h$')
            # for i in range(3):
            ...
            axs[sigma_].set_xlabel(r'$\sigma_h =$'+str(np.round(sigmas[sigma_]/sigma_x[0],decimals=1))+'$\sigma_x$', fontsize=18,color='black')
