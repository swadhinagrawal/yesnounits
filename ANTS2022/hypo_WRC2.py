...
if Fig1==1: #   Final Fig
    ...
    for i in mum_bxgh:
        ...
        axes_data = []
        for nop in range(len(number_of_opts_gxgh)):
            save_string = str(cnt)+'gxgh_mu_h_vs_mu_x_nop_'+str(number_of_opts_gxgh[nop])# str(cnt)+'gxgh_sx=sh_mu_h_vs_mu_x_vs_RCD_nop_'+str(number_of_opts_gxgh[nop]) #
            ...
            if isinstance(cbar,type(None))==True:
                ...
                cbar = fig.colorbar(cs,orientation="vertical",cax=cax,fraction=2)
                cbar.set_label(z_name,fontsize=12,fontweight='bold')
                font = {"fontsize":12,"fontweight":'bold'}
                cbar.set_ticks(np.arange(0,1.1,0.2),{"fontsize":8,"fontweight":'bold'})
            ...
            ax[nop].set_xlim(min(b),max(b))
            ax[nop].set_ylim(min(a),max(a))
            ax[nop].set_xlabel('Mean option quality $\mu_{q}$',fontsize = 14,fontweight='bold')
            ax[0].set_ylabel('Mean response threshold $\mu_{h}$',fontsize = 14,fontweight='bold')
            ...
            ax[nop].set_title(title,font,y=1.1,color='black')
            predicted_hrcc = prd.Hrcc_predict(delta_mu,'$\mu_{x_1}$',b,a,z,sigma_x_1,sigma_x_2,line_labels,prd.uniform,prd.ICPDF,1.0-(1.0/(line_labels)),prd.z_extractor,prd.optimization,line_labels)
            d = np.round(abs(predicted_hrcc[0][1]-intercept)/ np.sqrt(slope**2 +1),decimals=2)
            delta_slope = np.round(abs(predicted_hrcc[0][0]-slope),decimals=2)
            intercepts = [intercept,predicted_hrcc[0][1]]
...
def legend_func(func,bxgh_fit,uxgh_fit,gxgh_fit,func1,axes):
    ...
    leg3 = plt.legend(point_leg,dist_lab,loc='upper center', bbox_to_anchor=(0.5, 1.15),prop=dict(weight='bold',size=18),labelcolor=(0,0,0,1),ncol=3,columnspacing=2,frameon=False)
...
def legend_func_mum(func,bxgh_fit,uxgh_fit,gxgh_fit,func1,axes):
    ...
    # leg1 = plt.legend(point_leg,labels,loc='upper center', bbox_to_anchor=(0.5, 1.7),prop=dict(weight='bold',size=16),labelcolor=(0,0,0,1))
    leg3 = plt.legend(point_leg,dist_lab,loc='upper center', bbox_to_anchor=(0.5, 1.14),prop=dict(weight='bold',size=18),labelcolor=(0,0,0,1),ncol=3,columnspacing=2,frameon=False)
    # leg1 = plt.legend(point_leg,labels,loc='upper center', bbox_to_anchor=(0.5, 1.7),fontsize=12,prop=dict(weight='bold'),labelcolor=(0.3,0.3,0.3,1))
    # leg3 = plt.legend(point_leg,dist_lab,loc='upper center', bbox_to_anchor=(0.5, 1.1),fontsize=12,prop=dict(weight='bold'),labelcolor=(0.3,0.3,0.3,1),ncol=3,columnspacing=3,frameon=False)
    leg2 = plt.legend(point_leg1,labels1,loc='upper left', title="Swarm size\n"+r"   $S = m n$",bbox_to_anchor=(1, 1),title_fontsize=18,markerscale=1.5,prop=dict(size=20,weight='bold'),labelcolor=(0,0,0,1),frameon=False)
    # axes.add_artist(leg1)
    axes.add_artist(leg2)
    axes.add_artist(leg3)
...
if slopes_HARS ==1:
    ...
    # lab = [r'$\mu_h^{*}$','Sim.']
    # styles = ['-','--']
    # lines = [plt.plot([0, 0], [10, 10], c='black',linestyle=styles[i],label=lab[i],linewidth=5) for i in range(len(styles))]
    # plt.ylim(-2.5,5.5)
    # plt.legend(loc='lower left',markerscale=3,prop=dict(size=22,weight='bold'),labelcolor=(0,0,0,1),frameon=False)
    ...
    # bxgh_fit = genericPlotter(ax,distribution=1,color='slateblue',f_path=path+bxgh_string+'.csv',function='slope_fit',fit_type='no',first_variable='mum',vs_n = 1) #'log'
    # uxgh_fit = genericPlotter(ax,distribution=2,color='lightseagreen',f_path=path+uxgh_string+'.csv',function='slope_fit',fit_type='no',first_variable='mum',vs_n = 1)
    # gxgh_fit = genericPlotter(ax,distribution=3,color='coral',f_path=path+gxgh_string+'.csv',function='slope_fit',fit_type='no',first_variable='mum',vs_n = 1)
    ...
    # lines = [plt.plot([0, 0], [2, 2], c='black',linestyle=styles[i],label=lab[i],linewidth=5) for i in range(len(styles))]
    # plt.ylim(0,1)
    # plt.legend(loc='upper right',markerscale=4,prop=dict(size=18,weight='bold'),labelcolor=(0,0,0,1),frameon=False,labelspacing=10)
    ...
    # lines = [plt.plot([0, 0], [2, 2], c='black',linestyle=styles[i],label=lab[i],linewidth=5) for i in range(len(styles))]
    # plt.ylim(0,1)
    # plt.legend(loc='lower left',markerscale=4,prop=dict(size=18,weight='bold'),labelcolor=(0,0,0,1),frameon=False)
