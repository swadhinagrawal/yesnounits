...
class variableParams:
	def __init__(self):
		#   Option details
		self.num_opts = [2,5,8,10,15,20,30,40,80,100]
...
#		plt.pause(0.001)
	...
	def dissemination(self,robots,options,p):
		...
		while self.consensus == False:
			...
			if consensus_limit>1:
				num_communications =np.random.randint(low=1,high=range(len(yes_respondents)))
				talker = np.random.choice(range(len(yes_respondents)),num_communications,replace = False)[0]

				listener = np.random.choice(range(len(robots)),num_communications,replace = False)[0]

				output = parallel_wop(partial(self.dissemination_parallel,yes_respondents = yes_respondents, robots = robots,p = p),[(talker[i],listener[i]) for i in range(num_communications)])
				t += np.sum(np.array(output))
			elif consensus_limit==1:
	...
	def dissemination_parallel(self,talker,listener,yes_respondents,robots,p):
...
if __name__=="__main__":
	...
	fixed_params = [{'n': vP.num_opts[1], '$D_{m}$':vP.Dm[0],'$\mu_{m_{1}}$':vP.mu_m_1[1],'$\mu_{m_{2}}$':vP.mu_m_2[1],\
		'$\sigma_{m_{1}}$':vP.sigma_m_1[0],'$\sigma_{m_{2}}$':vP.sigma_m_2[0],'$D_{x}$':vP.Dx[0],\
		'$\mu_{x_{1}}$':vP.mu_x_1[70],'$\mu_{x_{2}}$':vP.mu_x_2[70],'$\sigma_{x_{1}}$':vP.sigma_x_1[10],\
		'$\sigma_{x_{2}}$':vP.sigma_x_2[10], '$D_{h}$':vP.Dh[0], '$\mu_{h_{1}}$': None, '$\mu_{h_{2}}$': None,\
		'$\sigma_{h_{1}}$':vP.sigma_h_1[10], '$\sigma_{h_{2}}$':vP.sigma_h_2[10],'$\delta_{\mu_{h}}$':vP.delta_mu_h}]
