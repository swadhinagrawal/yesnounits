...
if __name__=="__main__":
	...
	CDM_data_file = np.sort(np.array([f for f in os.listdir(path) if '.csv' in f and f[:3] == '478' and len(f)>10]))[0]
	...
	# mu_h_pred = vp.ICPDF(1-(1/2),List([mux[0],mux[0]]),stop,step,x,pdf)
	...
	smoothened = smooth(cdm_succ_rate[:-1],30)

	# axs1.plot([0]+muh,smoothened,c='red')
	axs1.plot(hypo_WRC2.mu_h,hypo_WRC2.vote_diff[:,6]/max(hypo_WRC2.vote_diff[:,6]),c='indigo')
	axs1.plot(hypo_WRC2.mu_h,hypo_WRC2.no_prop[:,6],c='skyblue')
