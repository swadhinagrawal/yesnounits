...
#    color = ['#004D40','#1E88E5']
    lable = [r"$q^{\bf '}$",r"$q^{\bf ''}$"]+["a" for a in range(5)]
    legendss = []
    for i in range(50):
...
if Fig2 == 1:
    ...
    plt.subplots_adjust(left=0.3,bottom=0.05, right=0.7, top=0.95, wspace=0.0,hspace=0.5)
    ...
    for j in range(len(var_mu)):
        ...
        for nop in range(len(number_of_options)):
            ...
            axs[nop].axes.get_xaxis().set_visible(False)
            slices = []
            mid_slices=[]
            ...
            axs1.bar(mid_slices,bins/100000,width=0.2,color='indigo')
            ...
            axs1.legend(loc='upper left',frameon=False,fontsize=16,labelspacing = 3)
...
#                    if sigma_+nop == 0:
#                        leg = QualityWiseOptions(rng.dx_u,low,high,number_of_options[nop],axs[sigma_],once=1)
#                    else:
#                        leg = QualityWiseOptions(rng.dx_u,low,high,number_of_options[nop],axs[sigma_],once=0)
                    if sigma_+nop == 0:
