...
normal_x_normal_h_compare_best_two = 1
...
def save_plot_data(analyzed_data,mum,num_opts,sig_q,save_string=None):
    f_path = path+save_string+'.csv'
...
def genericPlotter(ax,distribution,color,save_string,function,fit_type,first_variable,vs_n = 0,predicted = 0,plot_against_sig=0):
    f_path = path+save_string+'.csv'
...
if bimodal_x_normal_h==1:
    ...
    for i in mum_bxgh:
        ...
        for nop in number_of_opts_bxgh:
            ...
            save_string = str(cnt)+'bxgh_mu_h_vs_mu_x_nop_'+str(nop) #str(cnt)+'bxgh_sx=_sh_mu_h_vs_mu_x1_mu_x2_delta_mu_vs_RCD_nop_'+str(nop) # str(cnt)+
...
if bimodal_x_normal_h_plotter==1:
    ...
    for i in mum_bxgh:
        ...
        for nop in range(len(number_of_opts_bxgh)):
            save_string = str(cnt)+'bxgh_mu_h_vs_mu_x_nop_'+str(number_of_opts_bxgh[nop])#str(cnt)+'bxgh_sx=_sh_mu_h_vs_mu_x1_mu_x2_delta_mu_vs_RCD_nop_'+str(number_of_opts_bxgh[nop]) # str(cnt)+
            ...
            cs = ax.pcolormesh(np.array(b)+2.5,a,z,shading='auto')
            ...
            ax.set_xlim(min(np.array(b)+2.5),15)#max(np.array(b)+2.5)
            ax.set_ylim(2.5,max(a))
            ...
            vis.graphicPlot_paper(a,np.array(b)+2.5,x_name,y_name,z_name,title,save_name,z_var,fig, ax,cbar,[slope,intercept],hars,[predicted_hrcc[0]],[predicted_hrcc[1]],d,delta_slope,intercepts,linestyle=line_style[nop])
...
if uniform_x_normal_h==1:
    ...
    for i in mum_uxgh:
        ...
        for nop in number_of_opts_uxgh:
            number_of_options = nop
            save_string = str(cnt)+'uxgh_mu_h_vs_mu_x_nop_'+str(nop)#str(cnt)+'uxgh_sx=sh_mu_h_vs_mu_x_vs_RCD_nop_'+str(nop)#str(cnt)+
            # save_string,param = save_data(save_string,continuation)
            # if isinstance(param,type(None))==False:
            #     param.write("mu_m_1 : "+str(mu_m_1)+"\n")
            #     param.write("mu_m_2 : "+str(mu_m_2)+"\n")
...
if uniform_x_normal_h_plotter==1:
    ...
    for i in mum_bxgh:
        ...
        for nop in range(len(number_of_opts_bxgh)):
            ...
            vis.graphicPlot_paper(a,b,x_name,y_name,z_name,title,save_name,z_var,fig, ax,cbar,[slope,intercept],hars,[predicted_hrcc[0]],[predicted_hrcc[1]],d,delta_slope,intercepts,linestyle=line_style[nop])
...
if normal_x_normal_h==1:
    ...
    for i in mum_gxgh:
        ...
        for nop in number_of_opts_gxgh:
            ...
            hrcc,predicted_hrcc,a,b,x_name,y_name,title,save_name,z_var,line_labels,min_sig_h = vis.data_visualize(file_name=save_string+".csv",save_plot=save_string,x_var_='$\mu_{x_1}$',y_var_='$\mu_{h_1}$',cbar_orien="vertical",num_of_opts=nop,line_labels=number_of_options,z_var_='success_rate',plot_type='graphics',sigma_x_1=sigma_x_1,delta_mu=delta_mu,sigma_x_2=sigma_x_2,mu_m=mu_m_1,smoothened=1)
...
if normal_x_normal_h_compare_best_two==1:
    ...
    for i in mum_gxgh:
        ...
        for nop in number_of_opts_gxgh:
            ...
            vote_diff,no_prop = vis.data_visualize(file_name=save_string+".csv",save_plot=save_string,x_var_='$\mu_{x_1}$',y_var_='$\mu_{h_1}$',cbar_orien="vertical",num_of_opts=nop,line_labels=nop,z_var_='success_rate',plot_type='graphics',sigma_x_1=sigma_x_1,delta_mu=delta_mu,sigma_x_2=sigma_x_2,mu_m=mu_m_1,smoothened=1,vote_compare=1)
            
            vote_diff = np.array(vote_diff).reshape(len(mu_h),len(mu_x))
            no_prop = np.array(no_prop).reshape(len(mu_h),len(mu_x))
...
if normal_x_normal_h_sigma_plotter==1:
    ...
    cnt = 118#351#mu_h=mu_h* #156#mu_h=mu_x
    mum_slopes_gxgh = []
    num_slopes_gxgh = []
    # plt.style.use('ggplot')
    fig, ax = plt.subplots(1,3,figsize=(30,10),sharey=True)
    ...
    for i in mum_bxgh:
        ...
        for nop in range(len(number_of_opts_gxgh)):
            ...
            # ax.axes.get_xaxis().set_visible(False)
            ax.set_xlabel("Option qualities' std. dev. $\sigma_{q}$",fontsize = 16)
            ax[0].set_ylabel("Response thresholds' std. dev. $\sigma_{h}$",fontsize = 16)
            ax.tick_params(axis='both', which='major',labelsize=18)
            # ax.grid(b=True, which='major', color='black', linestyle='-',linewidth = 0.3,alpha=0.1)
            ax.minorticks_on()
            # ax.grid(b=True, which='minor', color='black', linestyle='-',linewidth = 0.2,alpha=0.1)
            ax.set_title(title,font,y=-0.25,color=(0,0,0,1))

            intercepts = [intercept]
            vis.graphicPlot_paper(a,b,x_name,y_name,z_name,title,save_name,z_var,fig, ax,cbar,[slope,intercept],hars,linestyle=line_style[nop],intercepts=intercepts)
        lines_1, labels_1 = ax[0].get_legend_handles_labels()
        lines_2, labels_2 = ax[1].get_legend_handles_labels()
        lines_3, labels_3 = ax[2].get_legend_handles_labels()
        lines = lines_1 + lines_2 + lines_3
...
if normal_x_normal_h_plotter_2==1:
    ...
    for i in mum_bxgh:
        ...
        for nop in range(len(number_of_opts_gxgh)):
            ...
            ax.set_xlim(min(b),max(b))
            ax.set_ylim(min(a),max(a))
            ax.set_xlabel('Mean option quality $\mu_{q}$',fontsize = 14)
            ...
            if nop==0:
                ...
                title = '(b) '+ title
            ax.set_title(title,font,y=1.05,color='black')
            predicted_hrcc = prd.Hrcc_predict(delta_mu,'$\mu_{x_1}$',b,a,z,sigma_x_1,sigma_x_2,line_labels,prd.uniform,prd.ICPDF,1.0-(1.0/(line_labels)),prd.z_extractor,prd.optimization,line_labels)
            d = np.round(abs(predicted_hrcc[0][1]-intercept)/ np.sqrt(slope**2 +1),decimals=2)
            delta_slope = np.round(abs(predicted_hrcc[0][0]-slope),decimals=2)
            intercepts = [intercept,predicted_hrcc[0][1]]
...
saving_sig = 0
if saving_sig:
    save_plot_data(np.array(mum_slopes_bxgh),mum_bxgh,np.ones_like(sig_q)*number_of_options,sig_q,save_string=bxgh_string)
    save_plot_data(np.array(mum_slopes_uxgh),mum_uxgh,np.ones_like(sig_q)*number_of_options,sig_q,save_string=uxgh_string)
    save_plot_data(np.array(mum_slopes_gxgh),mum_gxgh,np.ones_like(sig_q)*number_of_options,sig_q,save_string=gxgh_string)
...
if slopes_HARS ==1:
    ...
    # # plt.savefig(initials+"Intercept_n.eps",bbox_inches="tight",pad_inches=0.05)
    # plt.show()

    # plt.style.use('ggplot')
    fig, ax = plt.subplots()
    bxgh_fit = genericPlotter(ax,distribution=1,color='slateblue',save_string=bxgh_string,function='y_intercept_fit',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    uxgh_fit = genericPlotter(ax,distribution=2,color='lightseagreen',save_string=uxgh_string,function='y_intercept_fit',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    gxgh_fit = genericPlotter(ax,distribution=3,color='coral',save_string=gxgh_string,function='y_intercept_fit',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    # bxgh_fit = genericPlotter(ax,distribution=1,color='slateblue',save_string=bxgh_string,function='y_intercept_predicted',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    # uxgh_fit = genericPlotter(ax,distribution=2,color='lightseagreen',save_string=uxgh_string,function='y_intercept_predicted',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    # gxgh_fit = genericPlotter(ax,distribution=3,color='coral',save_string=gxgh_string,function='y_intercept_predicted',fit_type='a',first_variable='mum',vs_n = 1,predicted=2,plot_against_sig=1)
    # bxgh_fit = plot_inte_n(ax,'bxgh',color='slateblue',save_string=bxgh_string)
    # uxgh_fit = plot_inte_n(ax,'uxgh',color='lightseagreen',save_string=uxgh_string)
    # gxgh_fit = plot_inte_n(ax,'gxgh',color='coral',save_string=gxgh_string)
    ...
    plt.ylabel('y-intercept of\n HARS line in '+ylab1,fontproperties)
    plt.savefig(initials+"Intercept_sigma_q.pdf",bbox_inches="tight",pad_inches=0.05)
    plt.show()


    # # plt.style.use('ggplot')
    # fig, ax = plt.subplots()
    # bxgh_fit = genericPlotter(ax,distribution=1,color='slateblue',save_string=bxgh_string,function='slope_fit',fit_type='exp',first_variable='mum',vs_n = 1)
    # uxgh_fit = genericPlotter(ax,distribution=2,color='lightseagreen',save_string=uxgh_string,function='slope_fit',fit_type='exp',first_variable='mum',vs_n = 1)
    # gxgh_fit = genericPlotter(ax,distribution=3,color='coral',save_string=gxgh_string,function='slope_fit',fit_type='exp',first_variable='mum',vs_n = 1)
