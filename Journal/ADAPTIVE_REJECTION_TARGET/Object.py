...
class Object:
	def __init__(self,id,o_type,p,option=None):
		...
		if self.o_type == "R":
			...
			self.mario_update_step = np.random.uniform(0.001,0.1)
			self.personal_accepting_barrier = np.random.uniform(0.001,0.999)#np.random.choice(np.arange(0.0001,0.9999,0.0001),p=p.pdf_mario)
