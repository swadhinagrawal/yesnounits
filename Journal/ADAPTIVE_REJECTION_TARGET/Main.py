...
def looper(p,vP):
    ...
    for run in range(1000):
        ...
        # opt_assignment =  option_assignment(num_opts = p.num_opts, num_robo = p.num_opts*p.mu_m_1)
            ...
            if decision.threshold_update:
                decision.updateThresholds(robots,p,vP)
        # data = {'$\mu_{h_1}$':np.round(p.mu_h_1,decimals=3),'$\sigma_{h_1}$':np.round(p.sigma_h_1,decimals=3),'$x_{max}$ opt No.':0,'$x_{max}$':0,'$CDM$ opt No.':0,'$CDM$':0}
        data = {'$\mu_{h_1}$':np.round(p.mu_h_1,decimals=3),'$\sigma_{h_1}$':np.round(p.sigma_h_1,decimals=3),'$x_{max}$ opt No.':decision.ref_best,'$x_{max}$':options[decision.ref_best].quality,'$CDM$ opt No.':decision.best_option,'$CDM$':options[decision.best_option].quality}
        for i in range(len(robots)):
            # data[str(i)] = robots[i].threshold
            data[str(i)] = robots[i].personal_accepting_barrier
    ...
    if betaM:
        ...
        for n in vP.num_opts:
            ...
            fixed_params = [{'n': n, '$D_{m}$':vP.Dm[0],'$\mu_{m_{1}}$':vP.mu_m_1[1],'$\mu_{m_{2}}$':vP.mu_m_2[1],\
                '$\sigma_{m_{1}}$':vP.sigma_m_1[0],'$\sigma_{m_{2}}$':vP.sigma_m_2[0],'$D_{x}$':vP.Dx[2],\
                '$\mu_{x_{1}}$':14.5,'$\mu_{x_{2}}$':19.5,'$\sigma_{x_{1}}$':vP.sigma_x_1[9],\
