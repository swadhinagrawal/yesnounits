...
def units_u(mu_m,sigma_m,number_of_options):
      ...
      peak_choice = np.random.randint(0,len(mu_m),number_of_options)
      for i in range(len(peak_choice)):
            a = (mu_m[peak_choice[i]] - np.sqrt(3)*sigma_m[peak_choice[i]])   #   Lower bound of PDF
            b = (mu_m[peak_choice[i]] + np.sqrt(3)*sigma_m[peak_choice[i]])   #   Upper bound of PDF
            k = int(np.random.uniform(a,b))
            while k<=0:
                  k = int(np.random.uniform(a,b))
            a[i] = k      
      return a
...
def threshold_u(mu_h,sigma_h,m_units,h_type=3):
      ...
      peak_choice = np.random.randint(0,len(mu_h),m_units)
      for i in range(len(peak_choice)):
            a = (mu_h[peak_choice[i]] - np.sqrt(3)*sigma_h[peak_choice[i]])   #   Lower bound of PDF
            b = (mu_h[peak_choice[i]] + np.sqrt(3)*sigma_h[peak_choice[i]])   #   Upper bound of PDF
            a[i] = round(np.random.uniform(a,b),h_type)
      return a
...
def dx_u(mu_x,sigma_x,number_of_options,x_type=3):
      ...
      # peak_choice = np.append(peak_choice,np.array([0 for i in range(self.number_of_options-len(peak_choice))]))
      for i in range(len(peak_choice)):
            a = (mu_x[peak_choice[i]] - np.sqrt(3)*sigma_x[peak_choice[i]])   #   Lower bound of PDF
            b = (mu_x[peak_choice[i]] + np.sqrt(3)*sigma_x[peak_choice[i]])   #   Upper bound of PDF
            Dx[i] = round(np.random.uniform(a,b),x_type)
      return Dx
