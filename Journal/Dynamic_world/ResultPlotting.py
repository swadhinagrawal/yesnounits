...
if RT_distribution:
    ...
    for d in data_files[:]:
        data = pd.read_csv(path+d)
        num_opts.append(int(data.iloc[0]['n']))
    count = 0
    mean_mu_hs = []
    ...
    for d in data_files[:]:
        ...
        n_agents = int((len(data.iloc[0]) - 12)/4)
        rt_e = [data.iloc[-101]['tr_'+str(i)] for i in range(n_agents)]
        # rtdistribution_s = np.bincount(np.digitize(rt_s, bins),minlength=len(bins))

        start_tr = min(rt_e)-0.2
        end_tr = max(rt_e)+0.2
        x = np.arange(start_tr,end_tr,0.0001)
        bins = np.linspace(start_tr,end_tr,1000)
        ...
        for i in range(len(bins)):
            if i == 0:
                ax.plot([bins[i],bins[i]],[0,rtdistribution_e[i]],c='blue',linewidth=2,label=r'$D_{t_r}$ for '+str(n)+' opts')
            else:
        ...
        pdf_h1,_ = PDF(prd.gaussian,[1-1/n,1-1/n],[np.arctan(0.2/n),np.arctan(0.2/n)],x)
        ax.plot(x,pdf_h1/max(pdf_h1),c='slateblue',linewidth=3.5,label=r'$D_h$',linestyle='--')

        mean_x = List([data.iloc[0]['$\mu_{q_1}$'],data.iloc[0]['$\mu_{q_2}$']])
        sigma_x = List([1,1])

        step = 0.0001
        end = np.sum(mean_x)/len(mean_x) + 2*np.sum(sigma_x)+5
        start = np.sum(mean_x)/len(mean_x) - 2*np.sum(sigma_x)-5
        x = np.arange(start,end,0.0001)
        bins = np.linspace(start,end,1000)
        # thresholds_s = [data.iloc[0]['h_'+str(i)] for i in range(50*n)]
        thresholds_e = [data.iloc[-101]['h_'+str(i)] for i in range(n_agents)]
        # distribution_s = np.bincount(np.digitize(thresholds_s, bins),minlength=len(bins))
        distribution_e = np.bincount(np.digitize(thresholds_e, bins),minlength=len(bins))
        # mean_s = np.sum(thresholds_s)/len(thresholds_s)
        mean_e = np.sum(thresholds_e)/len(thresholds_e)
        ...
        for i in range(len(bins)):
            if i == 0:
                ax2.plot([bins[i],bins[i]],[0,distribution_e[i]],c='blue',linewidth=2,label=r'$D_{h}$ for '+str(n)+' opts')
            else:
        ...
        stop = np.sum(mean_x)/len(mean_x) + 2*np.sum(sigma_x)+5
        start = np.sum(mean_x)/len(mean_x) - 2*np.sum(sigma_x)-5
        if 'G' in data.iloc[0]['Dq']:
            pdf_x,area = PDF(prd.gaussian,mean_x,sigma_x,x)
            ax2.plot([np.sum(mean_x)/2,np.sum(mean_x)/2],[0,-max(pdf_x)],color='orange',linewidth = 2,linestyle='--',label=r'$\mu_q$')
            for i in range(1,n,1):
                ESM = prd.ICPDF(float(i)/n,mean_x,step,x,pdf_x)
                slices.append(np.round(ESM,decimals=3))
            sig_h = (0.07*np.log10(n)+0.57)*1
            mu_h = prd.ICPDF(1-(1/n),mean_x,step,x,pdf_x)
            pdf_h,_ = PDF(prd.gaussian,[mu_h,mu_h],[sig_h,sig_h],x)
            fn_slice = prd.gaussian

        elif 'K' in data.iloc[0]['Dq']:
            pdf_x,area = PDF(prd.gaussian,mean_x,sigma_x,x)
            ax2.plot([np.sum(mean_x)/2,np.sum(mean_x)/2],[0,-max(pdf_x)],color='orange',linewidth = 2,linestyle='--',label=r'$\mu_q$')
            sig_h = (0.05*np.log10(n)+0.58)*1
            mu_h = prd.ICPDF(1-(1/n),mean_x,step,x,pdf_x)
            pdf_h,_ = PDF(prd.gaussian,[mu_h,mu_h],[sig_h,sig_h],x)
            for i in range(1,n,1):
                ESM = prd.ICPDF(float(i)/n,mean_x,step,x,pdf_x)
        ...
        elif 'U' in data.iloc[0]['Dq']:
            pdf_x,area = PDF(prd.uniform,mean_x,sigma_x,x)
            ax2.plot([np.sum(mean_x)/2,np.sum(mean_x)/2],[0,-max(pdf_x)],color='orange',linewidth = 2,linestyle='--',label=r'$\mu_q$')
            sig_h = (-0.07*np.log10(n)+0.67)*1
            mu_h = prd.ICPDF(1-(1/n),mean_x,step,x,pdf_x)
            pdf_h,_ = PDF(prd.gaussian,[mu_h,mu_h],[sig_h,sig_h],x)
            for i in range(1,n,1):
                ESM = prd.ICPDF(float(i)/n,mean_x,step,x,pdf_x)
                slices.append(np.round(ESM,decimals=3))
            fn_slice =  prd.uniform
        ...
        for i in range(len(slices)+1):
            if i!=0 and i!=len(slices):
                x1 = np.arange(slices[i-1],slices[i],0.0001)
                pdf1 =  fn_slice(x1,mean_x,sigma_x)
                pdf1 = np.multiply(pdf1,1/area)
                ax2.fill_between(x1,0,-pdf1,facecolor=color[i])
            elif i==0:
                x1 = np.arange(start,slices[i],0.0001)
                pdf1 =  fn_slice(x1,mean_x,sigma_x)
                pdf1 = np.multiply(pdf1,1/area)
        ...
        pdf_x1,area = PDF(fn_slice,mean_x,sigma_x,bins)
        # hell_dis_s = Hellinger_distance(pdf_x1,distribution_s,bins)
        hell_dis_e = Hellinger_distance(pdf_x1,distribution_e,bins)
        ax2.plot(x,-pdf_x,color='orange',linewidth = 2,label=r'$D_q$')
        
        ax.legend(prop=dict(weight='bold',size=14),frameon=False)
        ax.set_xlim(start_tr,end_tr)
        plt.tight_layout()
        
        mean_mu_hs.append(np.sum(data.iloc[0::10]['$\mu_{h_1}$'])/len(data.iloc[0::10]['$\mu_{h_1}$']))
        ...
        icpdf = prd.ICPDF(1-1/n,mean_x,0.0001,x,pdf_x)
        ref_data.append(icpdf)
    
    ax1.plot(num_opts,mean_mu_hs)
