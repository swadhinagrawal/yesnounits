...
def looper(p,vP,path,mem_size,t_step):
    #   Gumbel distribution
    x_n = np.sqrt(0.5*lambertw((5**2)/(2*np.pi)))# 5 = num of opts, 0.5 = var_sigma, mean = 0.5
    n_rho_xn = x_n/0.5
    step = 0.0001
    start_g = 0
    stop_g = 1.0001
    ...
    robots = parallel(partial(obj.Object,o_type='R',p=p,mem_size=mem_size,options=options,pdf_g=np.flip(pdf_g.real)),range(p.num_robots))
    for j in range(3500):
...
if __name__=="__main__":
    ...
    for n in [5]:
        ...
        fixed_params = [{'n': n, '$D_{m}$':vP.Dm[0],'$\mu_{m_{1}}$':vP.mu_m_1[1],'$\mu_{m_{2}}$':vP.mu_m_2[1],\
			...
			'$\sigma_{x_{2}}$':vP.sigma_x_2[9], '$D_{h}$':None, '$\mu_{h_{1}}$': 20, '$\mu_{h_{2}}$': 20,\
