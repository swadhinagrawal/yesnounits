...
def looper(p,vP,path,mem_size,t_step):
    #   Gumbel distribution
    x_n = np.sqrt(0.5*lambertw((p.num_opts**2)/(2*np.pi)))# 5 = num of opts, 0.5 = var_sigma, mean = 0.5
    n_rho_xn = x_n/0.5
    step = 0.0001
    start_g = 0
    stop_g = 1.0001
    ...
    robots = parallel(partial(obj.Object,o_type='R',p=p,mem_size=mem_size,options=options,pdf_g=pdf_g.real),range(p.num_robots))
    for j in range(1000):
