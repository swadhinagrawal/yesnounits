...
path = os.getcwd() + "/results_memory_based/"
...
class VoterModel:
	def __init__(self,animator,robots,options,p):
		...
		self.iterations = 0
		for r in robots:
			if np.round(np.sum(r.memory)/len(r.memory),decimals=2)<np.round(r.threshold,decimals=2):
				r.response = 1#np.random.randint(low=0,high=2)
...
if __name__=="__main__":
	...
	fixed_params = [{'n': vP.num_opts[1], '$D_{m}$':vP.Dm[0],'$\sigma_{m_{1}}$':vP.sigma_m_1[0],\
		'$\sigma_{m_{2}}$':vP.sigma_m_2[0],'$D_{x}$':vP.Dx[0],'$\mu_{x_{1}}$':vP.mu_x_1[69],\
		'$\mu_{x_{2}}$':vP.mu_x_2[69],'$\sigma_{x_{1}}$':vP.sigma_x_1[9],'$\sigma_{x_{2}}$':vP.sigma_x_2[9],\
