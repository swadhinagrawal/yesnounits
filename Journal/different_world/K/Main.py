...
if __name__=="__main__":
    ...
    for n in [5]:
        ...
        fixed_params = [{'n': n, '$D_{m}$':vP.Dm[0],'$\mu_{m_{1}}$':vP.mu_m_1[1],'$\mu_{m_{2}}$':vP.mu_m_2[1],\
			...
			'$\mu_{x_{1}}$':vP.mu_x_1[99],'$\mu_{x_{2}}$':vP.mu_x_2[149],'$\sigma_{x_{1}}$':vP.sigma_x_1[9],\
			'$\sigma_{x_{2}}$':vP.sigma_x_2[9], '$\delta_{\mu_{x}}$':5, '$D_{h}$':None, '$\mu_{h_{1}}$': 20, '$\mu_{h_{2}}$': 20,\
			'$\sigma_{h_{1}}$':0, '$\sigma_{h_{2}}$':0,'$\delta_{\mu_{h}}$':0}]

        # memory_length = [2,10,20,40]#range(2,40,2)
        # threshold_step = [0.1,0.4,0.8,1]#np.arange(0.01,1.01,0.01)
        ...
        # #             vP.save_data(P,path)
        # #             looper(P,vP,path,mem_size,t_step)
        # for t_step in threshold_step:
        #     for times in range(20):
        #         P = paramObjects(fixed_params=fixed_params,vP_=vP)
        #         vP.save_params(fixed_params_column,fixed_params,path)
        #         vP.save_data(P,path)
        #         looper(P,vP,path,10,t_step)
