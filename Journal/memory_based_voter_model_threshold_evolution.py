...
path = os.getcwd() + "/results_evolving_memory_new/"
...
class variableParams:
	def __init__(self):
		...
		self.data_columns_name = ['n','$\mu_{m_1}$','$\sigma_{m_1}$','$\mu_{h_{pred}}$','$\sigma_{h_{pred}}$','$\mu_{h_1}$','$\sigma_{h_1}$','$x_{max}$ opt No.','$x_{max}$','$CDM$ opt No.','$CDM$','Iterations','no:yes']
...
class VoterModel:
	...
	def evolution_phase(self,robots,p):
		for r in robots:
			if np.sum(r.memory)/len(r.memory)<r.personal_accepting_barrier:
				r.threshold -= abs(np.sum(r.memory)/len(r.memory)-r.personal_accepting_barrier)
			else:
				r.threshold += abs(np.sum(r.memory)/len(r.memory)-r.personal_accepting_barrier)
...
class Object:
	def __init__(self,id,o_type,p):
		...
		if self.o_type == "R":
			self.personal_accepting_barrier = np.random.uniform(low= 0,high=1)
			self.patch = None
...
def looper(p):
	...
	for i in range(10):
		...
		model.evolution_phase(robots=robots,p=p)
		sum = 0
		for r in robots:
			sum += r.threshold
		muh = sum/len(robots)
